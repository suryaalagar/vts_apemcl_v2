    ...
    public function route_devation_cron()
    {
        ...
        $vehicle_list = DB::table('tripplan_reports AS tr')
            ->select(
                ...
                'l.ignition',
                'l.speed',
                'tp.radius AS deviationdistance',
                'tr.device_imei',
                'tr.vehicle_name',
                'l.device_updatedtime'
            )
            ->join('trip_polylines AS tp', 'tr.route_name', '=', 'tp.route_name')
        ...
        if (!empty($vehicle_list)) {
            ...
            foreach ($vehicle_list as $key) {
                $decodedPolyline = $this->decodePolyline($key->polyline);
                // dd($decodedPolyline);
                $yes = 0;
                // $route_id = $route_list->route_id;
                $radius = ($key->deviationdistance) == "" ? 100 : ($key->deviationdistance);
        ...
        if (!empty($complete_list)) {
            foreach ($complete_list as $list) {
                $playback = DB::table('play_back_histories')
                    ...
                    ->whereBetween('device_datetime', [$list->created_at, $list->updated_at])->get();
