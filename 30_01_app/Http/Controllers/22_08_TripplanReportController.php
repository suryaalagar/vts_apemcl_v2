...
class TripplanReportController extends Controller
{
    ...
    public function getData(Request $request)
    {
        ...
        foreach ($post_data as $data) {
            ...
            $tripId = $data->trip_id;
            $edit = '<button type="button" class="btn btn-success showModal"  onclick="test_function(' . "$tripId" . ');"  data-trip_id="' . $tripId . '">Map View</button>';
    ...
    public function planned_trips(Request $request)
    {
        ...
        $tripplan = DB::table('tripplan_reports AS t')
            ->select('t.device_imei', 't.vehicle_name', 't.route_name', 't.s_lat', 't.s_lng',
             't.e_lat', 't.e_lng', 'l.lattitute', 'l.longitute', 'l.angle',
             'l.ignition', 'l.speed','p.polyline',
